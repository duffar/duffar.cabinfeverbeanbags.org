...
function loadContent(){
    ...
    var webhome = 'https://dl.dropboxusercontent.com/u/14824619/CF/web';
    $('#vmarquee').load(webhome+'/raw_sponsors.html', function(){
        ...
        $('.category[name=gold] .sponsor').each(function(index){
            ...
            var $img = $s.attr('img');
            if (!$img) {
                $img = webhome+'/img/sponsors/'+$name+'.png';
            } else if (!$img.toLowerCase().startsWith("http")) {
                $img = webhome+'/img/sponsors/'+$img;
            }
